...
def extract_city_name(station_name):
    ...
    for i, (city, count) in enumerate(complete_city_counts[:15], 1):
    ...
    # 選擇重要城市（基於站點總數和完整站點數）
    important_cities = []
    for city, total_count in city_counts[:20]:  # 檢查前20個城市
        complete_count = len(complete_by_city.get(city, []))
        if complete_count > 0:
            important_cities.append((city, total_count, complete_count))
    
    # 按完整站點數排序
    important_cities.sort(key=lambda x: x[2], reverse=True)
    ...
    for city, total_count, complete_count in important_cities[:10]:
        ...
        for i, station in enumerate(city_complete_stations[:3], 1):  # 最多顯示3個
        ...
        if len(city_complete_stations) > 3:
            print(f"   ... 還有 {len(city_complete_stations) - 3} 個完整站點")
        print()
